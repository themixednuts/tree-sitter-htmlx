...
fn test_const_in_component_slot() {
    assert_eq!(
        ...
        "(document (element (start_tag name: (tag_name) (attribute name: (attribute_name (attribute_directive) (attribute_identifier)))) (const_tag (block_open) expression: (expression_value content: (js)) (block_close)) (expression content: (js)) (end_tag name: (tag_name))))"
    );
}
