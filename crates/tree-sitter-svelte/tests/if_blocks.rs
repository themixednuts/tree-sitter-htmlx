...
fn test_if_inside_element_content() {
    assert_eq!(
        ...
        "(document (element (start_tag (tag_name)) (block (block_start kind: (block_kind) expression: (expression)) (element (start_tag (tag_name)) (text) (end_tag (tag_name))) (block_end kind: (block_kind))) (end_tag (tag_name))))"
    );
}
...
fn test_else_if_block_kind_text() {
    ...
    assert_eq!(
        kind_text, "else if",
        "block_kind should be 'else if', not just 'else'"
    );
